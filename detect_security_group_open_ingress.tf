...
PATTERN
  ...
  environment_variable_map = { SNS_TOPIC = "arn:aws:sns:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:DetectSecurityGroupOpenIngress" }
